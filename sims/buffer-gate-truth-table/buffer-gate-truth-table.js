// Buffer Gate with Truth Table MicroSim
// Interactive buffer gate demonstration with truth table and Boolean expression
...
function setup() {
  ...
  describe('Interactive buffer gate with clickable input and truth table', LABEL);
}
...
function draw() {
  ...
  textSize(17);
  textStyle(BOLD);
  text('Buffer Gate', canvasWidth / 2, margin + 18);
  ...
  text('Y = A  (output equals the input, unchanged)', canvasWidth / 2, margin + 46);
  ...
  // Buffer gate body (triangle without bubble)
  fill('white');
  stroke(WIRE_COLOR);
  strokeWeight(2);
  triangle(
  ...
  // Data rows (only 2 rows for buffer gate)
  let truthRows = [[0, 0], [1, 1]];
  for (let r = 0; r < 2; r++) {
  ...
  textSize(14);
  textStyle(BOLD);
  text('Y = ' + output, tableX + tableW / 2, stateY + 18);
  ...
  textSize(13);
  textStyle(NORMAL);
  text('The buffer passes its input to the output unchanged.', canvasWidth / 2, boxY + 13);
  text('Buffers provide signal amplification, isolation, and timing delays.', canvasWidth / 2, boxY + 29);
}
