// 3-Input NAND Gate MicroSim
// Interactive 3-input NAND gate with truth table
...
function setup() {
  ...
  describe('Interactive 3-input NAND gate with clickable inputs and truth table', LABEL);
}
...
function draw() {
  ...
  let output = (inputs[0] & inputs[1] & inputs[2]) ? 0 : 1;
  let margin = 15;
  ...
  textSize(17);
  textStyle(BOLD);
  text('3-Input NAND Gate', canvasWidth / 2, margin + 18);
  ...
  text("Y = (A \u00b7 B \u00b7 C)'  (output is 0 only when all inputs are 1)", canvasWidth / 2, drawHeight - 100);
  ...
  strokeWeight(2);
  for (let i = 0; i < 3; i++) {
    line(gateX - gateW / 2 - 50, inYs[i], gateX - gateW / 2, inYs[i]);
  }
  ...
  text(output, outX + 45, gateY);
  fill('#333');
  textSize(11);
  textStyle(NORMAL);
  text('Y', outX + 45, gateY - 14);
  ...
  textSize(13);
  textStyle(NORMAL);
  text('A 3-input NAND outputs 0 only when A = 1 AND B = 1 AND C = 1.', canvasWidth / 2, boxY + 12);
  text('For n inputs: output is 0 only when ALL n inputs are 1.', canvasWidth / 2, boxY + 28);
}
