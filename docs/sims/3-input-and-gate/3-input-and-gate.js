// 3-Input AND Gate MicroSim
// Interactive 3-input AND gate with truth table
...
function setup() {
  ...
  describe('Interactive 3-input AND gate with clickable inputs and truth table', LABEL);
}
...
function draw() {
  ...
  textSize(17);
  textStyle(BOLD);
  text('3-Input AND Gate', canvasWidth / 2, margin + 18);
  ...
  text('Y = A \u00b7 B \u00b7 C  (output is 1 only when all inputs are 1)', canvasWidth / 2, drawHeight - 100);
  ...
  textSize(13);
  textStyle(NORMAL);
  text('A 3-input AND outputs 1 only when A = 1 AND B = 1 AND C = 1.', canvasWidth / 2, boxY + 12);
  text('For n inputs: output is 1 only when ALL n inputs are 1.', canvasWidth / 2, boxY + 28);
}
