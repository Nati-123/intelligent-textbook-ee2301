// 3-Input XOR Gate MicroSim
// Interactive 3-input XOR gate with truth table
...
function setup() {
  ...
  describe('Interactive 3-input XOR gate with clickable inputs and truth table', LABEL);
}
...
function draw() {
  ...
  textSize(17);
  textStyle(BOLD);
  text('3-Input XOR Gate', canvasWidth / 2, margin + 18);
  ...
  text("Y = A \u2295 B \u2295 C  (output is 1 when odd number of inputs are 1)", canvasWidth / 2, drawHeight - 100);
  ...
  strokeWeight(2);
  for (let i = 0; i < 3; i++) {
    line(gateX - gateW / 2 - 50, inYs[i], gateX - gateW / 2 - 2, inYs[i]);
  }
  ...
  textSize(13);
  textStyle(NORMAL);
  text('A 3-input XOR outputs 1 when an odd number of inputs are 1.', canvasWidth / 2, boxY + 12);
  text('XOR is associative: (A \u2295 B) \u2295 C = A \u2295 (B \u2295 C). Used in parity generation.', canvasWidth / 2, boxY + 28);
}
