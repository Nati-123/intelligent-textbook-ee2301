...
**Answer:** The correct answer is **B**. To convert from $\Sigma m$ to $\Pi M$, use all indices NOT in the minterm list. For 3 variables, indices range from 0 to $2^3 - 1 = 7$. If the ON-set (minterms) is $\{1,3,5\}$, the OFF-set (maxterms) is $\{0,2,4,6,7\}$. Therefore $F = \Sigma m(1,3,5) = \Pi M(0,2,4,6,7)$.
