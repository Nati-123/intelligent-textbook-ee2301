...
**Answer:** The correct answer is **B**. Comparing the DEMUX equation $Y_i = D \cdot m_i$ with the decoder-with-enable equation $Y_i = E \cdot m_i$, the expressions are identical when $D = E$. If the enable input carries a data signal, the device distributes that signal to one of $2^n$ outputs based on the select inputs (DEMUX behavior). If the enable is held at logic 1, all minterms pass through and the device simply decodes the address (decoder behavior). The 74138, for example, is sold as a "3-to-8 decoder/demultiplexer" because it serves both roles.
...
**Answer:** The correct answer is **C**. Evaluate $F$ for each select combination with $C = 0$ and $C = 1$:
...
**Answer:** The correct answer is **A**. Since each decoder output $Y_i$ equals the minterm $m_i$, implementing $F = \sum m(0, 3, 5, 7)$ requires connecting outputs $Y_0$, $Y_3$, $Y_5$, and $Y_7$ to a 4-input OR gate. The function is: $F = Y_0 + Y_3 + Y_5 + Y_7 = m_0 + m_3 + m_5 + m_7$. Option B lists the complement minterms (those NOT in the function), which would implement $\overline{F}$.
...
**Answer:** The correct answer is **D**. Applying the conversion formulas to binary $B_3B_2B_1B_0 = 0110$:
